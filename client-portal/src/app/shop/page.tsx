            ...
            .then((res) => {
                ...
                allProducts.forEach((product: any) => {
                    ...
                    if (product.generalAttributes) {
                        Object.entries(product.generalAttributes).forEach(
                            ([key, value]) => {
                                if (
                                    value !== null &&
                                    value !== undefined &&
                ...
                Object.entries(filters).forEach(([key, values]) => {
                    formattedFilters[key] = Array.from(values);
                });

                setAvailableFilters(formattedFilters);
            })
            .catch((err) => {
                console.error("Error extracting filters:", err);
                setAvailableFilters({});
            });
    }, [currentCategory]);
    ...
    const FilterSidebar = () => (
        <div className="space-y-10">
            ...
            <div className="space-y-4 relative">
                <h3 className="text-xs font-bold tracking-[0.15em] uppercase text-black/60">
                    Category
                </h3>
                ...
                >
                    ...
                    <ChevronDown
                        className={`w-4 h-4 transition-transform duration-300 ${isCategoryDropdownOpen
                                ? "rotate-180"
                                : ""
                            }`}
                    />
                ...
                {isCategoryDropdownOpen && (
                    <div className="absolute top-full left-0 w-full mt-2 bg-white border border-black/10 shadow-xl z-20 flex flex-col max-h-60 overflow-y-auto">
                        ...
                        <button
                            ...
                            className={`text-left px-4 py-3 text-xs tracking-widest uppercase hover:bg-black hover:text-white transition-colors ${!currentCategory
                                    ? "bg-black/5 font-black"
                                    : ""
                                }`}
                        >
                            All Categories
                        ...
                        {categories.map((cat) => (
                            <button
                                ...
                                className={`text-left px-4 py-3 text-xs tracking-widest uppercase hover:bg-black hover:text-white transition-colors ${currentCategory === cat._id
                                        ? "bg-black/5 font-black"
                                        : ""
                                    }`}
                            >
                ...
                <div className="flex flex-col gap-3">
                    {[
                        ...
                        return (
                            ...
                            >
                                <div
                                    className={`w-4 h-4 border flex items-center justify-center transition-colors ${isSelected
                                ...
                                <span
                                    className={`text-xs tracking-widest uppercase transition-colors ${isSelected
            ...
            {Object.keys(availableFilters).map((filterKey) => (
                ...
                >
                    ...
                    <div className="flex flex-wrap gap-2">
                        {availableFilters[filterKey].map(
                            (value) => {
                                const isSelected =
                                    searchParams.get(
                                        filterKey
        ...
        <div className="min-h-screen bg-white text-black">
            ...
            <div className="border-b border-black/10 py-16 px-6 lg:px-16 text-center bg-black/5">
                ...
                <p className="text-xs font-bold tracking-[0.2em] uppercase text-black/50 max-w-xl mx-auto leading-relaxed">
                    Explore our exclusive catalog of luxury
                    timepieces and fragrances, curated for the
                    modern aesthetic.
                </p>
            </div>
            ...
            <div className="w-full px-6 lg:px-16 py-12 flex flex-col lg:flex-row gap-12 lg:gap-16">
                ...
                {/* Mobile Overlay */}
                <div
                    className={`fixed inset-0 bg-black/60 z-50 transition-opacity duration-300 lg:hidden ${isMobileFilterOpen
                            ? "opacity-100"
                ...
                {/* Mobile Drawer */}
                <div
                    className={`fixed inset-y-0 right-0 z-50 w-[85%] max-w-md bg-white flex flex-col transition-transform duration-500 ease-in-out lg:hidden ${isMobileFilterOpen
                            ? "translate-x-0"
                            : "translate-x-full"
                        }`}
                >
                            ...
                            {products.map((product) => (
                                ...
                                >
                                    ...
                                    <div className="flex flex-col items-center text-center space-y-2">
                                        <h3 className="text-[11px] font-bold tracking-[0.15em] uppercase text-black/70 group-hover:text-black transition-colors">
                                            {product.name}
                                        </h3>
                        ...
                        <div className="flex justify-center items-center space-x-2 pt-16 border-t border-black/10 mt-16">
                            {Array.from(
                                ...
                                <button
                                    ...
                                    className={`w-10 h-10 flex items-center justify-center text-xs font-bold tracking-widest border transition-all ${currentPage === page
                                            ? "bg-black text-white border-black"
                                            : "bg-white text-black border-black/20 hover:border-black"
                                        }`}
                                >
...
export default function ShopPage() {
    return (
        <Suspense
            fallback={
                <div className="min-h-screen flex items-center justify-center text-xs tracking-widest uppercase font-bold">
                    Loading...
                </div>
            }
        >
